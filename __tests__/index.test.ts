...
describe('action', () => {
  ...
  describe('when nx project path is found', () => {
    ...
    it('should execute as expected', async () => {
      ...
      expect(setOutputMock).toHaveBeenCalledWith('affected-projects', 'project-name,lib-name');
    });
    ...
    describe('when project is not affected', () => {
      it('should set isAffected to false', async () => {
        ...
        expect(setOutputMock).toHaveBeenCalledWith('affected-projects', 'lib-name');
      });
    });

    describe('when getAffectedNxProjects fails', () => {
      it('should set a failed status', async () => {
        getAffectedProjectsMock.mockRejectedValue(new Error('Failed to get affected projects'));

        await index.run();

        expect(runMock).toHaveReturned();
        expect(findNxProjectPathMock).toHaveReturned();
        expect(setFailedMock).toHaveBeenCalledWith('Failed to get affected projects');
      });
    });
  });
